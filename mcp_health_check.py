#!/usr/bin/env python
"""
Polymarket v3 MCP Server Health Check Script

This script properly connects to the Polymarket v3 MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if Polymarket v3 MCP server is healthy"""
    print(f"🔍 Checking Polymarket v3 MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected Polymarket v3 tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check Polymarket v3 MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="Polymarket v3 MCP server URL (default: http://localhost:8000)")
    args = parser.parse_args()
    
    print(f"🚀 Polymarket v3 MCP Server Health Check")
    print(f"📍 Server URL: {args.url}")
    print(f"📰 Expected tools: example_tool")
    print("="*50)
    
    if check_mcp_server_health(args.url):
        print("\n✅ Polymarket v3 MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ Polymarket v3 MCP Server health check failed!")
        return 1
