# Polymarket v3 MCP Server

This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Polymarket v3 API. It enables AI agents and LLMs to interact with Polymarket v3 through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Polymarket v3 endpoints
...
- **Polymarket v3 Website**: [https://pma.d402.net](https://pma.d402.net)
- **API Documentation**: []()
...
*Note: Replace `example_tool` with actual Polymarket v3 API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/polymarket-v3-mcp-server.git
   cd polymarket-v3-mcp-server
   ```

2. Set your API key:
   ```bash
   export POLYMARKET_V3_API_KEY="your-api-key-here"
   ```
...
# Server's internal API key (for payment mode)
   POLYMARKET_V3_API_KEY=your-api-key-here
...
2. Run the server:
   ```bash
POLYMARKET_V3_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own Polymarket v3 API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_POLYMARKET_V3_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their Polymarket v3 API key
2. Server uses client's API key to call Polymarket v3 API
3. No payment required
4. Client pays Polymarket v3 directly
...
6. Server uses its INTERNAL Polymarket v3 API key to call the API
...
```bash
# Required
POLYMARKET_V3_API_KEY=your_internal_polymarket-v3_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Polymarket v3 API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Polymarket v3 endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/polymarket-v3-mcp-server",
  "mcp_server": {
    "name": "polymarket-v3-mcp",
    "description": "Polymarket mcp v3",
...
- `POLYMARKET_V3_API_KEY`: Your Polymarket v3 API key (required)
## Troubleshooting
