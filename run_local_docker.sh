...
# Script to build and run the Polymarket v3 MCP Server locally in Docker
...
IMAGE_NAME="polymarket-v3-mcp-server"
CONTAINER_NAME="polymarket-v3-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Polymarket v3 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Polymarket v3 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO

# API Authentication (server's internal key for payment mode)
POLYMARKET_V3_API_KEY=
...
if ! grep -q "^POLYMARKET_V3_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your Polymarket v3 API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "POLYMARKET_V3_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^POLYMARKET_V3_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^POLYMARKET_V3_API_KEY=.*|POLYMARKET_V3_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^POLYMARKET_V3_API_KEY=.*|POLYMARKET_V3_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "POLYMARKET_V3_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set POLYMARKET_V3_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ POLYMARKET_V3_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ Polymarket v3 MCP Server is running!${NC}"
echo
