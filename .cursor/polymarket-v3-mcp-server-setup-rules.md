# Polymarket v3 MCP Server Implementation Guide

You are working on implementing the Polymarket v3 MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: Polymarket v3
- **Documentation**: []()
- **Website**: [https://pma.d402.net](https://pma.d402.net)
- **Authentication**: Required - API key via `POLYMARKET_V3_API_KEY` environment variable
...
When a client provides their own Polymarket v3 API key:
...
Flow:
  1. Client connects with their Polymarket v3 API key
  2. MCP server uses client's API key to call Polymarket v3 API
  3. No payment required
  4. Client pays Polymarket v3 directly (not the MCP server)
```

**Use Case**: Clients who already have a Polymarket v3 subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL Polymarket v3 API key to call the API
  5. Client pays the MCP server (not Polymarket v3)
```

**Use Case**: Pay-per-use clients without their own Polymarket v3 subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("POLYMARKET_V3_API_KEY")
...
**Required**:
- `POLYMARKET_V3_API_KEY`: Server's internal Polymarket v3 API key (used when clients pay via 402)
...
POLYMARKET_V3_API_KEY=your_polymarket-v3_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_polymarket_v3",
      "get_polymarket_v3_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["polymarket v3", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the Polymarket v3 API with retry support. Example pattern:
...
def call_polymarket_v3_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call Polymarket v3 API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"Polymarket v3 API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_polymarket_v3(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search Polymarket v3 for [specific data type].
    ...
    try:
        # The call_polymarket_v3_api function already has retry logic
        results = call_polymarket_v3_api(
...
```python
@mcp.tool()
async def get_polymarket_v3_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_polymarket_v3_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in Polymarket v3.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_polymarket_v3(query="example", limit=5)
   ```
...
### 9. Common Polymarket v3 Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on Polymarket v3 capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the Polymarket v3 documentation that you should implement:
...
- Check the Polymarket v3 API documentation: 
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make Polymarket v3's capabilities easily accessible to AI agents through standardized MCP tools. 